...
- [ ] `templates.py` created with all functions
- [ ] `slugify` converts names correctly
- [ ] Environment uses `StrictUndefined`
- [ ] Templates load from configured path

**Gate**: Proceed to Step 2.2
...
- [ ] All 5 note templates created
- [ ] Templates produce valid YAML frontmatter
- [ ] Required fields present per STANDARDS.md
- [ ] Task format matches STANDARDS.md (📅, ⏫, 🔺, #task)
- [ ] `source` and `source_ref` included for traceability
- [ ] `generated` tag automatically added
- [ ] StrictUndefined catches missing variables

**Gate**: All checkboxes must pass before proceeding to Step 2.3
...
- [ ] All 4 profile YAML files created
- [ ] Profiles contain all required keys
- [ ] `load_profile` validates schema
- [ ] `select_profile` maps paths correctly per config
- [ ] Missing profile raises clear error
- [ ] Default falls back to "personal"

**Gate**: All checkboxes must pass before proceeding to Step 2.4
...
- [ ] `system-extractor.md.j2` created
- [ ] `system-planner.md.j2` created
- [ ] Prompts render with profile content
- [ ] Entity context injects correctly
- [ ] Date rules are clear
- [ ] Response format instructions are explicit

**Gate**: All checkboxes must pass before proceeding to Phase 3
...
- [ ] Returns a `note_type` for transcripts/emails
- [ ] Integrates with `select_profile()`
- [ ] Unit tests cover both inbox types
...
- [ ] `get_client()` uses environment variable
- [ ] `parse_structured()` returns Pydantic model
- [ ] `store=False` is always used (privacy)
- [ ] Retry logic with exponential backoff
- [ ] Metadata includes latency and tokens
- [ ] Missing API key raises clear error

**Gate**: All checkboxes must pass before proceeding to Step 3.2
...
- [ ] `extract.py` created with CLI interface
- [ ] `--file` extracts single file
- [ ] `--all` finds unprocessed files
- [ ] `--dry-run` shows planned actions
- [ ] Extraction JSON is valid `ExtractionV1`
- [ ] Skips already-processed files (has .extraction.json)
- [ ] Errors are logged, don't crash entire process

**Gate**: All checkboxes must pass before proceeding to Phase 4
...
- [ ] `plan.py` created with CLI interface
- [ ] Generates valid ChangePlan JSON
- [ ] Creates appropriate operations (create, patch, link)
- [ ] Patches README.md for entity with `last_contact`
- [ ] Handles new entities with warnings
- [ ] NO archive operations in output (those are deterministic)
- [ ] Validation catches forbidden ops/primitives
- [ ] Vault context is filtered (not entire vault)

**Gate**: All checkboxes must pass before proceeding to Phase 5
...
- [ ] `apply.py` created with CLI interface
- [ ] Backup created before modifications
- [ ] CREATE operations render templates correctly
- [ ] PATCH operations apply all primitives
- [ ] LINK operations add wikilinks
- [ ] Atomic writes prevent corruption
- [ ] Archive moves source correctly
- [ ] Git commit includes all changes
- [ ] Rollback restores backups on failure
- [ ] Rollback deletes created files on failure
- [ ] `--allow-dirty` bypasses clean check

**Gate**: All checkboxes must pass before proceeding to Phase 6
...
- [ ] `process_inbox.py` created with CLI interface
- [ ] Stages run in correct order: Extract → Plan → Apply
- [ ] Failed files go to `_failed/` with error logs
- [ ] Apply is atomic (all or nothing)
- [ ] Single git commit for entire batch
- [ ] `--dry-run` shows planned actions
- [ ] `--apply-only` skips extract/plan
- [ ] Re-run is idempotent (no duplicate processing)
...
- [ ] Logs are human-readable with `jq`

**Gate**: All Phase 6 checkboxes must pass before proceeding to Phase 7
...
- [ ] `standards_check.py` created
- [ ] Validates required frontmatter keys
- [ ] Validates date format (YYYY-MM-DD)
- [ ] Validates tag format (lowercase, hyphens only)
- [ ] Validates filename patterns
- [ ] Integration with apply phase
- [ ] Invalid content causes rollback (not write)

**Gate**: Proceed to Step 7.2
...
- [ ] Re-run detects already-processed files
- [ ] No duplicate notes created
- [ ] No duplicate git commits

**Gate**: Proceed to Step 7.3
...
- [ ] Tests can run without API calls (mocked)
- [ ] Test coverage for patch primitives
- [ ] Test coverage for rollback logic

**Gate**: All Phase 7 checkboxes must pass before proceeding to Phase 8
...
- [ ] `scanner.py` created
- [ ] Finds all entity folders
- [ ] Correctly identifies missing README
- [ ] Detects frontmatter issues
- [ ] Manifest JSON is valid and parseable
- [ ] Statistics are accurate

**Gate**: Proceed to Step 8.2
...
- [ ] `analyzer.py` created
- [ ] Creates README operations for missing folders
- [ ] Creates patch operations for missing keys
- [ ] Creates patch operations for wrong types
- [ ] Warns on unfixable issues (bad frontmatter)
- [ ] ChangePlan is valid Pydantic output

**Gate**: Proceed to Step 8.3
...
- [ ] `migrate.py` CLI wrapper created
- [ ] `executor.py` uses `TransactionalApply`
- [ ] Dry run shows planned changes without writing
- [ ] Apply is transactional (rollback on failure)
- [ ] `verifier.py` re-scans and reports compliance
- [ ] Full pipeline works end-to-end
- [ ] Migration is idempotent (running twice = no changes)

**Gate**: All Phase 8 checkboxes must pass before proceeding to Phase 9
...
- [ ] `Workflow/scripts/process_inbox.py`
- [ ] `Workflow/scripts/utils/logging.py`

### Phase 7: Hardening

- [ ] `Workflow/scripts/utils/standards_check.py`
- [ ] `Workflow/tests/__init__.py`
- [ ] `Workflow/tests/fixtures/transcripts/sample-meeting.md`
- [ ] `Workflow/tests/fixtures/expected/sample-meeting.extraction.json`
- [ ] `Workflow/tests/test_pipeline.py`

### Phase 8: Migration

- [ ] `Workflow/scripts/migration/__init__.py`
- [ ] `Workflow/scripts/migration/scanner.py`
- [ ] `Workflow/scripts/migration/analyzer.py`
- [ ] `Workflow/scripts/migration/executor.py`
- [ ] `Workflow/scripts/migration/verifier.py`
- [ ] `Workflow/scripts/migrate.py`
