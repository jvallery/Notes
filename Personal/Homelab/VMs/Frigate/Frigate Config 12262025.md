...
# Enable this only when troubleshooting LPR; very verbose:

# frigate.data_processing.common.license_plate: debug # see docs: https://docs.frigate.video/configuration/license_plate_recognition
...
# Frigate+ model (includes `face` and `license_plate`)

model:

path: plus://a3fd8c0d627ddfb65ffe15172de704ba

# ---- Audio events (0.16 built-in CPU detector) ----
...
listen: [speech, bark, yell] # limit to useful audio events for HA. :contentReference[oaicite:5]{index=5}

  

# (Removed bird classification: not tracking 'bird' => not used)
...
# -------- Objects tracked globally --------

# Keep the HA binary_sensors minimal — only useful labels.

# On specific cameras where cars/plates don't make sense, we override per-camera.
...
labels: [car, person, dog, cat, face, license_plate, package, amazon, ups,

fedex] # review UI only; not tracked => no HA entities
...
# -------- Face Recognition (0.16 schema) --------

face_recognition:

enabled: true

model_size: large # requires iGPU/dGPU; you have GPUs. :contentReference[oaicite:6]{index=6}

detection_threshold: 0.7 # used if face isn't a native label; with Frigate+ face is tracked, but harmless to keep
...
# -------- License Plate Recognition (0.16 schema) --------

lpr:

enabled: true # globally on; disabled per-camera where not needed

device: GPU # local recognition device
...
enhancement: 2 # can tune per camera

debug_save_plates: true # turn off once satisfied. :contentReference[oaicite:7]{index=7}

  

# ---- go2rtc restream (unchanged) ----
...
- path: rtsp://127.0.0.1:8554/skycam_west
...
backyard_suite_sidewalk_1: # no cars here
...
enabled: false # LPR OFF here

  

backyard_suite_sidewalk_2: # no cars here
...
backyard_wall_sidewalk: # no cars here
...
entryway_front_door: # no LPR here; focus on faces/people
...
garage_breezeway: # no cars/plates
...
patio_japanese_garden: # no cars/plates
...
storageyard_powerwall: # no cars/plates
