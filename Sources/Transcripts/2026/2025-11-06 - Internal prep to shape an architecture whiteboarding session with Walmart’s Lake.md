...
participants: Mikey, Brett, Jason Vallery, Matt, Avi, Lior, Jer, Paul, John Heidgerken,
  Siyash
tags:
- type/customer
- account/Walmart
...
**Date:** 2025-11-06 · **Customer:** Walmart · **Folder:** Customers/Walmart

> [!summary] Executive Summary
Internal prep to shape an architecture whiteboarding session with Walmart’s Lakehouse team. Walmart’s current posture: Azure for dot‑com systems, GCP (BigQuery/GCS) for enterprise analytics. They want to repatriate analytics to two on‑prem sites while keeping a consistent lakehouse view across GCP and both sites. Immediate ask: two VAST clusters (Region 1/2) in Q4 for pilot testing. Scale is extreme: initial ~450 PB, growing to ~770 PB by 2029; daily replication target ~10% of the lake from GCP to on‑prem (estimated sustained ~5 Tbps). Walmart previously floated 10 Tbps performance targets and large change rates. Two on‑prem DCs run active‑active with >30 ms latency. Walmart prefers GCS‑compatible APIs; S3 alone is not sufficient. Near‑term path likely uses SyncEngine for near‑real‑time replication plus DataSpaces across on‑prem sites; product gaps include GCS API compatibility and stronger consistency/write‑lease semantics. Team will compile targeted questions (POC scope, schemas, queries, governance, bandwidth) and try to schedule an in‑person session next week, pending Walmart stakeholder alignment.

## Stakeholders & Roles
Mikey, Brett, Jason Vallery, Matt, Avi, Lior, Jer, Paul, John Heidgerken, Siyash

## Key Facts (Context, constraints, signals)
- Walmart uses Azure for dot‑com and GCP (BigQuery/GCS) for analytics
- Goal: hybrid lakehouse with consistent view across GCP and two on‑prem Walmart sites
...
- Daily replication target: ~10% of lake from GCP to on‑prem; estimated ~5 Tbps sustained
- Prior figures mentioned: up to ~10 Tbps performance targets; high monthly change rates
- Two on‑prem DCs with >30 ms latency; deep fiber interconnects (capacity TBD)
- Workloads include Trino/Presto, Spark; tables in Delta/Hudi; BigQuery access in GCP
- Walmart favors GCS‑compatible API; S3 alone not acceptable long‑term
...
- Agreed to prepare a targeted question list for Walmart’s Lakehouse team
- Plan an in‑person architecture whiteboarding session with Walmart (aiming next week, pending scheduling)
- Align internal approach: near‑term SyncEngine replication from GCS plus DataSpaces across on‑prem sites
- Acknowledge product gaps to evaluate: GCS API endpoint, stronger consistency/write leases, GCS change‑feed integration
...
- Network throughput may be insufficient for sustained multi‑Tbps replication from GCP to on‑prem
- Strong consistency across GCP and two on‑prem sites is non‑trivial with current capabilities
- Product gaps (GCS API, write‑lease semantics, change‑feed integration) could delay timelines
- POC scope and performance targets are immature/fantastical without workload details
- Missing governance/multi‑tenancy and compliance requirements may cause rework
...
- Governance: multi‑tenancy, RBAC/ABAC, auditing, data residency/compliance requirements
- Network plan: provisioned bandwidth and paths between GCP and each DC; egress model
- Per‑site capacity/performance targets and whether both on‑prem sites must hold full replicas
...
- [x] Compile and send consolidated architecture questions to Walmart (Vandana) by EOD @Jason Vallery ⏫ 📅 2025-11-06 ✅ 2025-11-08
- [x] Secure the right Walmart stakeholders (Lakehouse, vector DB) and coordinate the design session @Mikey #walmart ⏫ ✅ 2025-11-08
- [x] Follow up via email with answers to prior cloud questions raised in Slack @Lior 🔼 ✅ 2025-11-08
- [x] Align with Alon and Andy on SyncEngine pattern and lessons from the Wave project @Avi 🔼 ✅ 2025-11-08
- [x] Produce Q4 configurations for two VAST clusters (Region 1 and Region 2) for Walmart pilot @Paul ⏫ ✅ 2025-11-08
- [x] Draft pilot test plan framework and required data/artifacts list (with sample query set) @Matt 🔼 ✅ 2025-11-08
- [x] Validate GCS change‑feed support path for SyncEngine and outline required engineering work @Avi ⏫ ✅ 2025-11-08
- [x] Extract insights from Walmart tech blog links to inform architecture and test plan @Jason Vallery 🔽 ✅ 2025-11-08

### Follow‑Ups (Customer / Us / Partner)
- [x] Provide POC scope: datasets, schemas, sample data, and representative Trino/Spark queries @Vandana (Walmart) ⏫ ✅ 2025-11-08
- [x] Confirm network bandwidth plan and paths between GCP and both on‑prem DCs @Walmart ⏫ ✅ 2025-11-08
- [x] Deliver governance requirements: tenancy model, access controls, auditing, data residency @Walmart ⏫ ✅ 2025-11-08
- [x] Clarify object format standards (Delta/Hudi) and BigQuery interoperability needs @Walmart 🔼 ✅ 2025-11-08
- [x] Confirm API requirement and acceptance for GCS‑compatible endpoint and timeline @Walmart 🔼 ✅ 2025-11-08
- [x] Agree pilot success criteria (RPO/RTO, query SLAs, throughput targets) @Walmart 🔼 ✅ 2025-11-08
- [x] Confirm availability for in‑person architecture whiteboard session (target next week) @Vandana (Walmart) 🔼 ✅ 2025-11-08
