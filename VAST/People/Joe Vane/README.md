...
TASK
FROM ""
WHERE !completed AND contains(text, "Joe Vane")
SORT due ASC
```
...
```dataview
TASK
WHERE contains(text, "@JoeVane") AND !completed
SORT due ASC
```
...
**Role**: BizDev at Microsoft (BizDev)
**Relationship**: Target Microsoft BizDev contact

**Background**:
- BizDev contact; reports to Harish; recommended path to educate and use to secure exec sponsorship and route to Ronnie Booker’s org.
