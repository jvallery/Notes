---
type: people
title: Brian Moore
created: '2026-01-03'
last_contact: '2025-12-18'
auto_created: true
tags:
- type/people
- needs-review
email: pootzwacke@gmail.com
company: VAST Data
---

# Brian Moore

## Profile

**Role**: _Unknown_
**Organization**: _Unknown_
**Location**: _Unknown_
**Relationship**: _Unknown_

## Open Tasks

```dataview
TASK
FROM this.file.folder
WHERE !completed AND contains(text, "Brian Moore")
SORT due ASC
```

## Recent Context

- 2025-12-01: Email re: Re: RFE Alert: RFE 0482 - Existing Feature Enhance - Brian Evans from VAST Data has submitted and approved an RFE for NVIDIA Corporation, requesting enha

- 2025-12-17: Email re: RFE Alert: RFE 0526 - Existing Feature Enhancement - Brian Evans has submitted and approved an RFE for an existing feature enhancement related to Cisco,

- 2025-12-18: Email re: Re: Early heads up - board game the week after Chr - Brian Bearly is organizing a board game session the week after Christmas and is coordinating with fr

- 2025-12-17: Email re: RFE Alert: RFE 0526 - Existing Feature Enhancement - Brian Evans has submitted and approved an RFE for an existing feature enhancement related to Cisco,
## Key Facts

- Brian Bearly is hosting a board game session at his place.
## Background

_Career history, expertise, interests, personal details shared..._

## Key Decisions

## Related

---
*Last updated: 2025-10-27*

- [[NVIDIA Corporation]]
- [[DGX-C | Coreweave]]
- [[RFE 0482]]

- [[Cisco]]

- [[church]]

- [[Utility space]]
- [[TMIMITW]]

- [[Tues mens group]]