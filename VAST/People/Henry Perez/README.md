...
TASK
FROM ""
WHERE !completed AND contains(text, "Henry Perez")
SORT due ASC
```
...
```dataview
TASK
WHERE contains(text, "@HenryPerez") AND !completed
SORT due ASC
```
...
**Background**:
- Google contact Jason Vallery will soft-reach to assess path to Midjourney and appropriateness for intros; noted as potentially dicey due to hyperscaler conflicts.
