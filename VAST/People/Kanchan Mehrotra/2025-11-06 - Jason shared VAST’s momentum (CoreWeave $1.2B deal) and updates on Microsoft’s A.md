...
created: { { DATE } }
status: "done"
counterpart: "[[Kanchan Mehrotra]]"
role: ""
team: ""
company: ""
series: "1-1/Kanchan Mehrotra"
cadence: "Weekly"
meeting_mode: "Video"
location_or_link: ""
calendar_url: ""
...
privacy: "internal"
ai_extracted: true
transcript_path: "00 Inbox/Transcripts/20251106 1259 Parallels Transcription.txt"
tags: [meeting, "1-1"]
---

# 1:1 — Kanchan Mehrotra — 2025-11-06
...
Jason shared VAST’s momentum (CoreWeave $1.2B deal) and updates on Microsoft’s Apollo: a VAST POC rack is en route, and Azure asked to run VAST bare metal on Azure storage hardware (specs with Fungible DPU or NVIDIA BlueField 3). They discussed leveraging this for MAI (Dallas capacity in Dec/Apr) and UK Met, political dynamics with Azure Storage (Manish), and a path to co-engineer a VAST-optimized Azure storage SKU with Michael Myrah. Jason outlined NVIDIA DGX Cloud storage requirements where VAST is qualified and Azure Storage lags. They agreed Kanchan will test appetite with Kushal, seed the story with Nidhi, and pursue an exec meeting with Renan, with Myrah engagement after Nidhi’s buy-in.

## Key facts learned

- VAST signed a ~$1.2B software licensing deal with CoreWeave.
- Apollo leadership (Chi, CVP) asked VAST to: (1) run a POC on VAST hardware (rack shipped to an Azure DC), and (2) run VAST bare metal on Azure storage hardware; specs include Fungible DPU and NVIDIA BlueField 3 options.
- Fungible DPU maturity is a risk; no production storage tenants use it yet despite prior milestones.
- MAI is Apollo’s first customer; long-term vision is multi-tenant, 3P-configurable offering.
- MAI Dallas capacity: initial tranche in December, larger tranche in April; storage plan for April is still fluid and Kushal is interested in VAST.
...
- DGX Cloud on Azure today uses AMLFS for H100/A100; no GB-series storage benchmarking yet observed by Kanchan.
...
- Michael Myrah (Azure Storage Hardware, under Long’s team) owns the storage hardware stack; potential partner for a VAST-optimized SKU.
- VAST prefers an ‘all-you-can-eat’ enterprise license with Microsoft over piecemeal per-capacity deals.

## Outcomes

- Agreement for Kanchan to probe Kushal’s willingness to pursue VAST for MAI Dallas and navigate the politics.
- Alignment to seed the VAST-on-Azure-storage bare-metal story with Nidhi and pursue an exec meeting including Renan.
- Consensus to approach Michael Myrah after securing Nidhi’s support.
- Recognition that NVIDIA DGX storage specs can be used as leverage with Azure Storage; Kanchan to inquire internally.

## Decisions

- Jason will not contact Michael Myrah directly to avoid premature escalation; Kanchan to open that thread when appropriate.
- Primary path is through Azure Storage hardware (Myrah) for a single, unified story; Azure Dedicated (Anand) considered as a parallel option if needed.

## Action items (for Kanchan Mehrotra)

- [x] Reach out to Kushal Bhatta to assess MAI’s appetite for VAST as storage for Dallas (esp. April) and understand current POR hardware. @Kanchan Mehrotra ⏫ ✅ 2025-11-08
- [x] Seed the VAST-on-Azure-storage bare-metal strategy with Nidhi and secure a meeting including Renan. @Kanchan Mehrotra ⏫ ✅ 2025-11-08
- [x] Coordinate with Lior and Renan to request/prepare for an exec meeting with Nidhi (pre/post Ignite). @Jason Vallery 🔼 ✅ 2025-11-08
- [x] Ask Azure Storage team if they have received NVIDIA DGX storage performance requirements and any related benchmarking requests. @Kanchan Mehrotra 🔼 ✅ 2025-11-08
- [x] Draft a plan for VAST bare metal on classic Azure (Overlake/SDN interoperability, networking) pending MAI greenlight. @Jason Vallery ⏫ ✅ 2025-11-08
- [x] Finalize and circulate the exabyte cost/power/performance comparison slide for exec discussions. @Jason Vallery 🔼 ✅ 2025-11-08
- [x] Open a conversation with Michael Myrah about a VAST-optimized Azure storage SKU after securing Nidhi’s support. @Kanchan Mehrotra 🔼 ✅ 2025-11-08
- [x] Explore Azure Dedicated (Anand) as a parallel bare-metal path if needed. @Kanchan Mehrotra 🔽 ✅ 2025-11-08

## Follow-ups

- [x] Confirm with Kushal whether MAI will formally explore VAST for Dallas April capacity and clarify POR hardware. @Kanchan Mehrotra ⏫ ✅ 2025-11-08
- [x] Track Apollo POC progress (rack arrival, VAST on Azure storage hardware, DPU choice between Fungible and BlueField 3). @Jason Vallery 🔼 ✅ 2025-11-08
- [x] Schedule and confirm the meeting with Nidhi and Renan; align on engagement model and licensing. @Kanchan Mehrotra ⏫ ✅ 2025-11-08
- [x] Determine Azure’s stance on adopting NVIDIA DGX storage performance requirements. @Kanchan Mehrotra 🔼 ✅ 2025-11-08

## Risks

- Political resistance from Azure Storage leadership (Manish) to adopting VAST.
- Dependency on Fungible DPU amid historical delivery slippage.
- Tight timeline to make April Dallas capacity if engineering for classic Azure bare-metal isn’t started promptly.
- Exec availability (Ignite) may delay alignment.
- Neo-cloud deployments without adjacent storage may limit future workload flexibility.

## Open questions

- Will MAI endorse VAST as the storage solution for the Dallas April tranche?
- Can VAST be qualified to run bare metal on Azure storage hardware (classic Azure) in time for Dallas?
- Which DPU will Apollo select for testing and rollout: Fungible or NVIDIA BlueField 3?
- Will Nidhi support engaging Michael Myrah to co-develop a VAST-optimized Azure storage SKU?
- Is Azure Storage willing to meet NVIDIA DGX Cloud storage performance requirements, and on what timeline?
- What hardware POR is slated for MAI Dallas (Dec and April tranches)?
- Can an exec meeting with Nidhi and Renan be scheduled around Ignite or shortly after?
