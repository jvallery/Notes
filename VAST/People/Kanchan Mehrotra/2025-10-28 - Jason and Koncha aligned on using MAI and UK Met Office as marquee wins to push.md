...
privacy: "internal"
ai_extracted: true
transcript_path: "00 Inbox/Transcripts/20251028 1604 Parallels Transcription.txt"
tags: [meeting, "1-1"]
---

# 1:1 — Koncha — 2025-10-28
...
Jason and Koncha aligned on using MAI and UK Met Office as marquee wins to push Azure toward a VAST-suitable hardware shape and unlock broader collaboration. Near-term plan: Jason meets MAI (Kushal) Friday, prepares a comparative economics deck (VAST vs Blob and LSV4/LSV5/on‑prem), and both work UK Met Office and Supercomputing touchpoints (Egal keynote slide, booth narrative). Marketplace offer is progressing but won’t unlock major co-sell until better VM shapes arrive; sales motions remain secondary until tangible product paths exist.
...
- LSV5 is committed by Egal’s team but roughly a year+ out; current networking plans may still be insufficient for UK Met Office price/perf targets.
- VAST density advantage: ~10x fewer racks (≈240 racks blob vs ≈20 racks VAST for 1 EB) and ~1/5 power in MAI Falcon-like scenarios.
- MAI (ex‑Inflection) has existing VAST affinity; champions include Kushal and Vipin Sachdeva.
- UK Met Office is a strong target but constrained by SKU/networking economics.
- Prior art: Azure NetApp Files model (partner hardware in Azure) is a viable pattern for VAST (via OEM/ODM).
- Neo cloud deployments need GPU-adjacent storage for network disconnect resilience; target ratio of local storage per ~8k GPUs is desired.
...
- 3P GPU demand remains high but constrained by supply; sales motion exists but is gated by capacity and tangible product offers.
- Marketplace control plane from Yancey’s team acquired by VAST; Google first, Azure to follow (internal tracking suggests early next year).

## Outcomes

- Agreement to prioritize MAI and UK Met Office as flagship wins to drive Azure hardware alignment.
- Plan to meet at Supercomputing to coordinate story, customer touchpoints, and potential keynote content.
- Consensus to keep marketplace progressing while pursuing special hardware shapes via leadership.
- Agreement to leverage Nidhi as an internal advocate and schedule a review meeting with her.
...
## Action items (for Koncha)

- [x] Meet Kushal (MAI) Friday to discuss VAST opportunities and clarify the "not really Azure" capacity hint. @Jason Vallery 🔺 ✅ 2025-11-08
- [x] Reconnect with Vipin Sachdeva (MAI) to re‑engage VAST. @Jason Vallery ⏫ ✅ 2025-11-08
- [x] Build internal comparative deck for a 1 EB deployment (VAST on LSV4/LSV5/on‑prem vs Blob HDD/Flash) to support the hardware shape ask. @Myself 🔺 ✅ 2025-11-08
- [x] Coordinate a review with Nidhi to walk through the deck and align on the MAI + UK Met Office plan. @Koncha ⏫ ✅ 2025-11-08
- [x] Meet UK Met Office stakeholders (Mike Kiernan, Nico, Allen) at Supercomputing to push the VAST path and surface LSV5 networking needs. @Jason Vallery ⏫ ✅ 2025-11-08
- [x] Speak with Egal to push on LSV5 shape and networking/bandwidth requirements and align timelines. @Jason Vallery ⏫ ✅ 2025-11-08
- [x] Align the joint VAST–Azure story for Supercomputing/Ignite (booths/panel) with Andrew, Joe Green, and Lior. @Koncha 🔼 ✅ 2025-11-08
- [x] Check if Suresh will attend Supercomputing; if yes, set up a Neo cloud storage discussion with Jason; if not, schedule later. @Koncha 🔼 ✅ 2025-11-08
- [x] Provide input/content for Egal’s keynote slide referencing VAST. @Jason Vallery 🔼 ✅ 2025-11-08
- [x] Progress the Azure Marketplace offer (initial LSV4-based) and track ETA; be ready to brief sellers once credible. @VAST PM team 🔼 ✅ 2025-11-08

## Follow-ups

- [x] Clarify MAI’s incremental opportunity and capacity location after the Friday meeting. @Jason Vallery ⏫ ✅ 2025-11-08
- [x] Validate GPU‑adjacent storage ratio guidance for Neo clouds with Suresh/Anand and explore Microsoft usage models (lease vs 1P vs Azure-sold). @Koncha 🔼 ✅ 2025-11-08
- [x] Keep Kurt informed so sellers remember VAST when qualifying 3P GPU deals. @Jason Vallery 🔽 ✅ 2025-11-08
- [x] Track Wave’s 40 PB request and assess a viable Azure configuration or alternatives; report back. @Jason Vallery 🔼 ✅ 2025-11-08
- [x] Confirm Nidhi’s Ignite/Supercomputing schedule and lock a time to review the deck. @Koncha 🔼 ✅ 2025-11-08

## Risks

- Azure Storage conflict of interest and limited incentive to support third‑party storage.
- LSV5 timeline and networking may not meet customer economics, delaying traction.
- Marketplace on current SKUs may disappoint customers and harm credibility.
- 3P GPU capacity constraints limit near‑term co‑sell opportunities.
- MAI plans may rely on non‑Azure capacity, complicating alignment.
- Internal alignment across stakeholders (Egal, Nidhi, leadership) could slow execution.

## Open questions

- What is MAI’s exact "not really Azure" capacity plan and the incremental opportunity for VAST?
- How will Microsoft use Neo cloud capacity (lease/provider vs 1P deployment vs Azure-sold), and what contracting model would enable VAST?
- Can Azure commit to qualifying a hyper‑dense hardware shape (Overlake‑style model) for VAST, and who will sponsor it?
- What are the final LSV5 networking specs and timelines, and can bandwidth be increased to hit UK Met Office targets?
- Does Wave still want a 40 PB VAST marketplace solution on Azure, and is there a viable configuration with current SKUs?
- When can Nidhi review the comparative deck and align on the MAI + UK Met Office approach?
- Who will own creating and socializing the seller differentiation story internally once the offer is credible?
