...
TASK
FROM ""
WHERE !completed AND contains(text, "Robert Brooks")
SORT due ASC
```
...
```dataview
TASK
WHERE contains(text, "@RobertBrooks") AND !completed
SORT due ASC
```
...
**Background**:
- Key connector to Giannis at Reflection; currently hard to get a meeting with (non-responsive).
