...
Reconnected post-sabbatical and aligned on Jason’s initial focus: evaluate OpenAI’s AI caching approach and its suitability for MAI at extreme scale. Discussed MAI requirements (100k-node scale, AKS/Spark), alternative cache options (BlobFuse/Blockfuse, AC Store, Alexio/DAX), and Blob/Bifrost direction. Jason will verify OpenAI IP/code access, meet MAI and Ong on Friday, and start a technical evaluation. Jai will share docs (OpenAI IP note, MAI friction doc, Apollo) and connect Jason with Lukasz on Bifrost.

## Key facts learned

- MAI target scale: ~400k GPUs for training and ~40k for inferencing in ~2 years
- Cache must scale to ~100,000 nodes and integrate with AKS/Kubernetes and Spark
- Options under consideration: BlobFuse/Blockfuse, AC Store, Alexio/DAX (incl. KB cache), OpenAI cache/IP
- Preference for a single caching solution usable for both training and inference (pluggable, not tied to one framework)
- OpenAI may provide IP/code usable across Microsoft services, pending legal confirmation (via Pete and SILA legal)
- Bifrost adds a direct read path bypassing FE/table layers; Lukasz is implementing parts of this
- Potential MAI constraint: multi‑region pooling for a distributed cache (to be confirmed)
- Jason to shift focus toward engineering deep dives and technical evaluations

## Outcomes

- Initial assignment set: Jason to evaluate OpenAI’s caching solution and feasibility for MAI
- Agreement to verify OpenAI IP/code access before code review
- Plan to gather MAI requirements updates and status on clusters
- Jai to share relevant documents (OpenAI IP note, MAI friction doc, Apollo) and connect Jason with Lukasz
...
- Jason to prioritize OpenAI cache evaluation while tracking other options (BlobFuse/Blockfuse, AC Store, Alexio/DAX)

## Action items (for Jai Menon)

- [x] Evaluate OpenAI caching code for feasibility, architecture (unified vs separate caches), performance/scale (~100k nodes), and AKS/Spark fit; produce a recommendation. @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Confirm OpenAI IP rights and request code access with Pete and SILA legal; ensure Jason has access. @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Send Jason the OpenAI IP agreement details referenced in prior discussions. @Jai Menon 🔼 ✅ 2025-10-26
- [x] Discuss the performance snapshot outcome with Ong; decide on escalation to Manish. @Jason Vallery ⏫ 📅 2025-09-05 ✅ 2025-10-26
- [x] Meet MAI (lunch) to confirm requirements (GPU counts, multi‑region need, cluster status, training framework plans). @Jason Vallery ⏫ 📅 2025-09-05 ✅ 2025-10-26
- [x] Review latest Blockfuse/BlobFuse progress and Nagendra’s 50‑page document; sync with the team. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Compare options (BlobFuse/Blockfuse, AC Store, Alexio/DAX) against MAI scale and AKS/Spark integration; capture pros/cons. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Send MAI 'frictions with Microsoft infra' 10‑page document to Jason. @Jai Menon 🔼 ✅ 2025-10-26
- [x] Send Apollo document to Jason. @Jai Menon 🔼 ✅ 2025-10-26
- [x] Connect Jason with Lukasz regarding Bifrost direct read path. @Jai Menon 🔼 ✅ 2025-10-26
- [x] Sync with Lukasz to understand Bifrost direct path and applicability to MAI workloads. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Draft an AI caching requirements/options document (unified cache, training-first; inference KB later). @Jai Menon 🔼 ✅ 2025-10-26
- [x] Walk Jai through the agentic coding workflow demo when time permits. @Jason Vallery 🔽 ✅ 2025-10-26

## Follow-ups

- [x] Establish a regular 1‑1 cadence between Jason and Jai. @Jason Vallery 🔽 ✅ 2025-10-26
- [x] After IP confirmation, obtain and review OpenAI cache codebase. @Jason Vallery ⏫ ✅ 2025-10-26

## Risks

- Unclear IP/legal access to OpenAI code could delay evaluation
- Scaling to ~100k nodes and meeting MAI performance targets may exceed current solutions
- Fragmentation risk if multiple caches are pursued for different use cases
- Integration complexity with AKS/Spark at MAI scale
- MAI dissatisfaction with Microsoft infra indicates gaps that may require broader changes
- Ambiguity around multi‑region cache requirements increases design complexity

## Open questions

- Do we have firm legal clearance and access to OpenAI cache code, and which components are available?
- Is OpenAI’s caching unified across training and inference/KB, or separate implementations?
- Can the OpenAI cache scale to ~100,000 nodes and meet MAI latency/throughput targets?
- How well would OpenAI’s cache integrate with AKS/Kubernetes and Spark at MAI scale?
- Will MAI use OpenAI’s training framework, and what frameworks should the cache prioritize?
- Are MAI GPUs distributed across regions, requiring a cross‑region distributed cache?
- What is the status of MAI’s first GPU storage clusters and current Blob experience?
- What are the latest performance/scale claims for Blockfuse/BlobFuse, AC Store, and Alexio/DAX?
- What is Bifrost’s finalized scope/timeline, and how does the direct read path impact MAI workloads?
- What regular 1‑1 rhythm should Jason and Jai set?
