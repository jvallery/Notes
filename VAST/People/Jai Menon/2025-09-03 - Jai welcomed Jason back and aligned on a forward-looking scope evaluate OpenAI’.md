...
Jai welcomed Jason back and aligned on a forward-looking scope: evaluate OpenAI’s cache code as part of defining Microsoft’s AI caching strategy for MAI. Target is a single, pluggable cache supporting both training and inference, scaling to 100k nodes on AKS/Spark. Jason will confirm IP/code access with Pete/SILA, assess technical fit and scalability, and sync with MAI and related teams (BlockFuse/C-Store, Alluxio/DAX, Bifrost). Jason will also discuss the disappointing snapshot with Ong (and possibly Manish).

## Key facts learned

- Goal: a single, pluggable caching solution for training and inference (preferably one cache).
- MAI scale targets in ~2 years: 400k GPUs for training (~100k nodes) and 40k GPUs for inference.
- Primary environment: AKS/Kubernetes with Spark.
- Options under consideration: C-Store proposals (Krishnan’s team), Alluxio/DAX (now supports inference/KB caching), OpenAI cache code (likely accessible under MS IP terms), BlockFuse/BlobFuse approaches.
- OpenAI cache access appears permitted for Microsoft services; needs confirmation via Pete and SILA legal.
- Bifrost adds a direct read path from compute to capacity nodes (bypassing FE/table for reads).
- Lukasz is contributing Bifrost direct path; can brief Jason.
- Compute for MAI moved under Brendan’s org (AKS); CVP Qiu Ke (Kiki) involved; Yumin coordinating.
- Potential MAI requirement: multi-region pooling for a distributed cache (to be confirmed).
- Near-term platform focus: Bifrost and a distributed cache; DeltaZero positioned as follow-on.

## Outcomes

- Initial assignment: Jason to evaluate OpenAI’s cache for viability, scale, and AKS/Spark fit.
- Agreement to pursue a single pluggable cache design across training and inference (training prioritized first).
- Jai will send OpenAI IP details, MAI feedback doc, and Apollo materials.
- Jason to reconnect with MAI and related teams and establish a regular check-in rhythm with Jai.
- Jason will address performance snapshot feedback with Ong and potentially Manish.

## Decisions

- Evaluate OpenAI cache as a first concrete step.
- Prioritize training use cases for MAI; add inference/KB caching support subsequently.
- Target deployment environment: AKS + Spark; must scale to ~100k nodes.

## Action items (for Jai Menon)

- [x] Confirm IP and repository access to OpenAI cache code with Pete and SILA; obtain code access. @Jason Vallery 🔺 ✅ 2025-10-26
- [x] Evaluate OpenAI cache code: architecture, performance, and viability for a unified training/inference cache; assess scale to ~100k nodes and fit with AKS/Spark. @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Review MAI 10-page feedback and Apollo docs once received; extract requirements relevant to caching and storage. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Sync with MAI (Ong) to verify cluster status, timelines, and potential multi-region cache pooling requirements. @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Re-engage with BlockFuse/BlobFuse team; review Nagendra’s doc and current performance/scale claims. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Connect with Lukasz to understand Bifrost direct read path design and interfaces. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Send OpenAI IP agreement details and request code access if needed. @Jai Menon ⏫ ✅ 2025-10-26
- [x] Send MAI 10-page frustrations document and Apollo materials to Jason. @Jai Menon ⏫ ✅ 2025-10-26

## Follow-ups

- [x] Meet Wamsi to discuss the performance snapshot and context. @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Discuss snapshot outcome with Ong; decide whether to speak with Manish. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Establish a regular 1-1 cadence with Jai. @Jason Vallery 🔽 ✅ 2025-10-26

## Risks

- Uncertain IP/code access and licensing constraints for OpenAI cache.
- Scalability to 100k nodes and multi-region pooling may be challenging.
- Integration complexity across AKS/Spark and varied training/inference frameworks.
- Blob latency/throughput may remain a bottleneck without Bifrost improvements.
- Cross-team dependencies (MAI, AKS, legal, partner teams) could delay progress.
- Morale/engagement risk due to disappointing snapshot if not addressed.

## Open questions

- Is Microsoft fully authorized to use and adapt OpenAI’s cache code for MAI services (scope, repos, support)?
- Is OpenAI’s cache unified across training and inference or separate implementations?
- Can the chosen solution scale to ~100k nodes and integrate cleanly with AKS/Spark?
- Does MAI require multi-region cache pooling for early clusters?
- What is the current status of MAI GPU clusters and Blob performance with them?
- Which training/inference frameworks should be prioritized first (e.g., NVIDIA Dynamo)?
