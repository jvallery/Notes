...
TASK
FROM ""
WHERE !completed AND contains(text, "Noah Yonick")
SORT due ASC
```
...
```dataview
TASK
WHERE contains(text, "@NoahYonick") AND !completed
SORT due ASC
```
...
**Role**: Data science at Perplexity
**Relationship**: Target contact (prospect)

**Background**:
- Perplexity target contact; possible intro via Grant Lee; also connected to Rory Kellerworthy.
