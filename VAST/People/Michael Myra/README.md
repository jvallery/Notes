...
TASK
FROM ""
WHERE !completed AND contains(text, "Michael Myra")
SORT due ASC
```
...
```dataview
TASK
WHERE contains(text, "@MichaelMyra") AND !completed
SORT due ASC
```
...
**Role**: Microsoft (Azure Storage Hardware and Roadmap (per transcript))
**Relationship**: Microsoft contact (potential backchannel)

**Background**:
- Friend of Jason; reports to Ong; involved in qualifying storage hardware SKUs; potential but politically sensitive outreach path.
