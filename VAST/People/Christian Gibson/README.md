...
TASK
FROM ""
WHERE !completed AND contains(text, "Christian Gibson")
SORT due ASC
```
...
```dataview
TASK
WHERE contains(text, "@ChristianGibson") AND !completed
SORT due ASC
```
...
**Role**: Thinking Machines
**Relationship**: Target contact (prospect)

**Background**:
- Previously worked with Jason Vallery at OpenAI; moved to Thinking Machines; Glenn Lockwood likely has stronger relationship.
