---
type: people
email: ''
company: VAST Data
title: Eirikur Hrafnsson
last_contact: '2025-10-28'
created: '2026-01-05'
tags:
- type/people
- needs-review
role: VP of Engineering for Cloud Engineering
expertise:
- GCP MVP deployment flow
- networking and failover options
- GCP Marketplace private offers
- implementation coordination
relationship: colleague
enrichment_level: 2
last_enriched: '2026-01-05'
---

# Eirikur Hrafnsson

## Key Facts

- Eirikur Hrafnsson is the VP of Engineering for cloud engineering at VAST Data and is described as a co-founder at Green Cloud.

- Eirikur Hrafnsson owned implementing the GCP MVP deployment flow using routable IPs and requiring a customer-provided IP range.

- Eirikur Hrafnsson owned asking Google about maintenance overlap guarantees across failure domains and documenting guidance.

- Eirikur Hrafnsson will drive Tackle.io implementation coordination for GCP Marketplace private offers, including confirming overage feasibility with Tackle.io and providing finance access to GCP Marketplace reporting.
## Recent Context

- 2025-10-29: Mentioned in: Microsoft and VAST align parallel tracks: UK Met Azure LSV4 test cluster and Microsoft-hosted bare-metal dev system

- 2025-10-28: Google GCP networking: VIP failover options and RDMA constraints for Z4M

- 2025-10-28: Mentioned in: Cloud BU leadership aligns on dual-track GTM: GCP Marketplace MVP launch plus hyperscaler-scale MAI storyline

- 2025-10-28: Mentioned in: GCP Marketplace MVP launch readiness, private offers via Tackle, fixed capacity pricing and Polaris entitlements
## Open Tasks
```tasks
path includes Eirikur Hrafnsson
not done
```
