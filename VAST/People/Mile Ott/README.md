...
TASK
FROM ""
WHERE !completed AND contains(text, "Mile Ott")
SORT due ASC
```
...
```dataview
TASK
WHERE contains(text, "@MileOtt") AND !completed
SORT due ASC
```
...
**Role**: AI/infra (contact target) at Thinking Machines
**Relationship**: Target contact (prospect)

**Background**:
- Identified as key AI/infra decision-maker; reportedly talking to CoreWeave and others.
