...
| Field | Value |
|-------|-------|
| **Role** | Finance/capacity leader (manager of Melissa Du) |
| **Company** | OpenAI |
...
TASK
FROM ""
WHERE !completed AND contains(text, "Kevin Park")
SORT due ASC
```
...
```dataview
TASK
WHERE contains(text, "@KevinPark") AND !completed
SORT due ASC
```
...
**Role**: Finance/capacity leader (manager of Melissa Du) at OpenAI (Finance/capacity)
**Relationship**: Customer stakeholder (finance/capacity)

**Background**:
- Leads finance/capacity area referenced in relation to 'neo clouds'/CoreWeave logistics.
