...
privacy: "internal"
ai_extracted: true
transcript_path: "00 Inbox/Transcripts/20251029 1102 Parallels Transcription.txt"
tags: [meeting, "1-1"]
---
...
Jason introduced his cloud-first vision for VAST (VAST-as-a-Service, multi-cloud, GPU-adjacent plus central storage) and interviewed Rick on how CS operates for CoreWeave and XAI. Rick runs a managed-services/SRE-style pod for these on-prem hyperscale customers. XAI and CoreWeave mostly operate on-prem clusters; XAI stores massive raw training data in object clusters and previously used VAST for checkpointing/inferencing. XAI asked for a single large cluster spanning multiple clusters; VAST supports S3 sync replication and site-to-site global namespace but not a single filesystem across clusters. Common issues are networking and C/D-node failures; RMAs are handled via VAST depots. Communication is primarily via Slack with tickets in Salesforce/Jira and strict SLAs (e.g., ~30 min for Sev1) with legal penalties; Gordon Brown is the CSM for CoreWeave. Staffing includes weekday onsite at XAI and APJ/EMEA after-hours coverage with a goal of proactive incident notification.

## Key facts learned

- Rick is tech lead for two of VAST’s largest customers: CoreWeave (#1) and XAI (#3).
- Both customers run primarily on-prem; XAI may hold cloud data but brings it down to VAST clusters for work.
- XAI requested one large cluster across multiple clusters; VAST cannot present a single filesystem across clusters.
- VAST supports synchronous replication for S3 buckets and a site-to-site global namespace (not across disparate clusters).
- XAI object clusters are ~314 PB used per cluster, rapidly filling with raw training data (e.g., YouTube, Instagram).
- Checkpointing usage decreased recently but some activity has returned in Colossus.
- CS engagement is managed-services/SRE-like with close daily collaboration via Slack and rapid response expectations.
- Common issues: networking, C-node and D-node failures; deeper Kubernetes/CSI questions route to vForce/R&D.
- Hardware RMAs go to VAST depots (Campbell or Sacramento); field work favors whole-node swaps, limited FRU replacements (fans).
- Primary comms via Slack; urgent issues move to live calls; tickets in Salesforce (CS) and Orion/Jira (vForce).
- Formal SLAs/SLOs exist with legal penalties; example cited ~30-minute response for Sev1 (confirm with Gordon Brown).
- Staffing: weekday onsite at XAI; APJ/EMEA provides off-hours coverage; weekends lighter but monitored; aim for proactive alerts.

## Outcomes

- Shared understanding of current on-prem managed-services model for CoreWeave and XAI.
- Identified limitations around multi-cluster single-filesystem requests and current replication/namespace options.
- Jason to treat Rick as the go-to workload expert for XAI and stay in touch.
- Need to review SLA/SLO and penalty details with Gordon Brown.
...
- [x] Connect with Gordon Brown to review CoreWeave/XAI SLAs, response targets, and penalties. @Jason Vallery ⏫ ✅ 2025-11-08
- [x] Draft an initial operating model for VAST-as-a-Service (SRE vs. CS roles, on-call, comms, SLAs). @Jason Vallery 🔼 ✅ 2025-11-08
- [x] Deep-dive with Rick on XAI workload patterns (checkpointing, GPU-adjacent caching, data flows) to inform cloud design. @Jason Vallery 🔼 ✅ 2025-11-08
- [x] Assess feasibility/roadmap options for multi-cluster single-namespace/filesystem needs raised by XAI. @Jason Vallery 🔼 ✅ 2025-11-08

## Follow-ups

- [x] Obtain and review the legal SLA document and any penalty clauses for CoreWeave/XAI. @Jason Vallery ⏫ ✅ 2025-11-08
- [x] Validate current after-hours and weekend coverage model across APJ/EMEA for critical incidents. @Jason Vallery 🔼 ✅ 2025-11-08
- [x] Confirm details of XAI’s cloud data location and workflows (e.g., Google) and how data is repatriated. @Jason Vallery 🔽 ✅ 2025-11-08
- [x] Review Slack alarm channel configurations and escalation paths for proactive outreach. @Jason Vallery 🔽 ✅ 2025-11-08

## Risks

- Small team coverage strain for 24x7 managed-services expectations.
- Legal penalty exposure tied to strict SLAs/SLOs.
- Networking instability and C/D-node failures impacting training jobs.
- Weekend and off-shift knowledge transfer gaps for complex changes/installs.
- Current inability to present a single filesystem across multiple clusters may block customer asks.
- Very rapid data growth on XAI object clusters.

## Open questions

- What are the exact SLA/SLO response and resolution targets and associated penalties?
- What is the definitive location/provider for XAI’s cloud-resident data and how is it integrated?
- Can VAST’s roadmap support a multi-cluster single-filesystem or enhanced global namespace to meet XAI’s request?
- What is the formal on-call rotation and escalation model across regions, including weekends and holidays?
- How will the managed-services/SRE approach evolve for a multi-tenant VAST-as-a-Service offering?
