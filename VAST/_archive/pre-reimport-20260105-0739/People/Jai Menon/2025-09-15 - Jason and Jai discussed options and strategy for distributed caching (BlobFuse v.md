...
Jason and Jai discussed options and strategy for distributed caching (BlobFuse vs open source vs internal Manifold/TensorCache), MAI’s current approach (local NVMe + async copy), and immediate steps to evaluate BlobFuse PP and alternative caches. Jai confirmed the Oct 15 Silicon Valley offsite and will centralize materials; Jason will test builds and shape a scenario-based recommendation.
...
- Goal: align on distributed cache strategy that covers fan-out writes, fan-out reads, fan-in reads, and separately KV cache.
- MAI currently writes checkpoints to local NVMe and asynchronously copies to Blob; simplicity valued; checkpoints tolerate partial loss.
- BlobFuse PP today targets fan-out writes; limited/no read caching; some scale testing done (100-node CycleCloud sim).
- Inferencing team expressed interest in BlobFuse but hasn’t received a usable build for ~2 months.
- Options under consideration: build/own, fork an open source (e.g., Alluxio, JuiceFS), or adopt internal solutions (Manifold/Singularity, OpenAI TensorCache).
- Manifold reportedly outperforms OpenAI TensorCache in shared benchmarks; Manifold uses consistent hashing (no central metadata store).
- OpenAI’s TensorCache reportedly moved away from a metadata store toward hashing; active churn expected as GPT-6 focuses on memory/long context.
- Concerns to validate: consistent hashing scalability/rebalancing, whether to build a high-TPS metadata/index store, Go+FUSE performance vs C++/kernel client, and Alluxio IP/Java considerations.
...
- Agreed to centralize all distributed cache docs in a shared Teams room.
- Plan for Jason to evaluate BlobFuse PP build and compare with TensorCache and Manifold.
- Plan to set up a deep-dive with the Manifold team (via Vipul) and collect collateral (slides, benchmarks).
- Jai will explore bringing inferencing (Rajat Monga) and DPU folks into the Oct 15 offsite agenda.
...
- [x] Book travel to Silicon Valley for the Oct 15 team offsite @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Create a Teams room to centralize distributed cache materials and share with the team (incl. Jean and Jagan) @Jai Menon ⏫ ✅ 2025-10-26
- [x] Upload and share Whipple/Manifold slides and related comparison links in the Teams room @Jai Menon ⏫ ✅ 2025-10-26
- [x] Schedule a Manifold/Singularity deep-dive session and invite the team @Vipul ⏫ ✅ 2025-10-26
- [x] Provide BlobFuse PP build, deployment guide, and 100-node perf results @Vishnu ⏫ 📅 2025-09-19 ✅ 2025-10-26
- [x] Evaluate BlobFuse PP (fan-out writes) and benchmark against MAI needs; also assess TensorCache and Manifold @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Draft scenario-based framing (fan-out writes, fan-out reads, fan-in reads, KV cache) and update the doc @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Prepare an MAI-focused proposal comparing lazy copy vs BlobFuse on performance, complexity, and risk @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Ensure the inferencing team receives the BlobFuse build and evaluates it @Jai Menon 🔼 ✅ 2025-10-26

## Follow-ups

- [x] Get a clear answer on why MAI cannot use BlobFuse today and what changes would unblock adoption @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Validate Manifold’s consistent hashing approach for scale-in/scale-out and rebalancing costs @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Decide metadata strategy: build a high-TPS index store vs rely on open-source (e.g., FoundationDB) @Jai Menon ⏫ ✅ 2025-10-26
- [x] Assess Alluxio IP/China and Java stack implications; determine viability to fork and own @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Clarify North Star: build vs open source vs internal, and if/when to pursue hardware offload/kernel-mode path @Jai Menon 🔼 ✅ 2025-10-26
- [x] Confirm BlobFuse language/runtime choices (Go, FUSE) meet perf targets; define plan if C++/kernel client is needed @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Confirm Oct 15 offsite agenda and external speakers (Rajat Monga, DPU/GPU teams) @Jai Menon 🔽 ✅ 2025-10-26
- [x] Obtain current stability/roadmap for OpenAI TensorCache and any impact of GPT-6 memory focus @Jason Vallery 🔽 ✅ 2025-10-26

## Risks

- BlobFuse PP delays and missed handoffs to key partners (e.g., inferencing team).
- MAI may not adopt BlobFuse absent clear performance and simplicity benefits.
- Go+FUSE may not meet performance targets, implying costly shifts to C++/kernel.
- Consistent hashing approaches may complicate scaling and rebalancing.
- Relying on open source (e.g., Alluxio) raises IP, support, and Java stack concerns.
- OpenAI TensorCache churn may limit its suitability as a reference or dependency.
- High effort and longer timelines if pursuing deep hardware offload/kernel-mode path.

## Open questions

- Specifically why can’t MAI use BlobFuse today?
- How does Manifold’s consistent hashing handle cluster scale-in/out and data rebalancing?
- Should we build our own high-TPS metadata/index store or adopt an open-source one?
- Will Go+FUSE meet our performance needs, or do we need a C++/kernel client?
- Are Alluxio’s IP/China and Java concerns blockers for us or customers?
- Which scenarios should our first delivery prioritize (fan-out writes, fan-out reads, fan-in reads, KV cache)?
- Will MAI adopt if we show clear performance and simplicity gains vs lazy copy?
- Do we pursue deep hardware offload/kernel-mode integration now or later?
- How stable is OpenAI TensorCache and is churn acceptable for us to depend on?
- What are the detailed 100-node BlobFuse PP performance numbers and limits?

> Next meeting (if any): 2025-10-15
