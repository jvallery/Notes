...
privacy: "internal"
ai_extracted: true
transcript_path: "00 Inbox/Transcripts/20250903 1459 Teams Meeting (Parallels) Transcription.txt"
tags: [meeting, "1-1"]
---
...
Jai outlined a high-priority need to define an AI caching strategy for MAI at massive scale. Options include OpenAI’s cache, Blobfuse/Blockfuse, AC Store/C-Store, and Alexio/DAX. Jason will start by evaluating the OpenAI cache (IP access, code quality, scalability to 100k nodes, fit with AKS/Spark) and compare against alternatives. Bifrost remains the near-term performance focus for Blob (with a new direct read path), with DeltaZero as a follow-on. Jai will send relevant docs and connect Jason with key people.

## Key facts learned

- MAI target scale: ~400,000 GPUs for training (~100,000 nodes) and ~40,000 GPUs for inferencing in ~2 years
- Primary environment: AKS/Kubernetes and Spark
- Caching options under consideration: OpenAI cache, Blobfuse/Blockfuse, AC Store/C-Store (Krishnan), Alexio/DAX
- Preference for a single, pluggable cache serving both training (including checkpoint/find data) and inference (KB cache) over multiple bespoke caches
- OpenAI reportedly permits Microsoft to use their IP (including cache); needs confirmation via Pete and SILA legal
- Bifrost focus: improved latency/throughput; adds a direct read path from compute to capacity nodes (bypassing FE/table) for many reads
- Lukasz is building parts of Bifrost, including the direct path, with Vishwajith on Jagan’s team
- Compute for AI moved to Brendan’s org (AKS); CVP Qi Ke leads compute/AKS support for MAI; Yumin engaged on our side
- Jason to evaluate OpenAI cache code quality, scalability, and AKS/Spark fit; compare to Blobfuse/Blockfuse, AC Store/C-Store, and Alexio/DAX
- MAI may need multi-region logical pooling for cache; to be confirmed
- Nagendra shared a ~50-page doc on progress; Krishnan has proposals; Blockfuse sharing performance numbers
- Jason to talk with Ong Friday about snapshot; may also speak with Wamshi and Manish

## Outcomes

- Agreed initial assignment: Jason to evaluate the OpenAI cache for MAI’s requirements
- Jai to send OpenAI IP note, Apollo doc, and MAI frustrations doc (from Ong) to Jason
- Jason to confirm IP access with Pete and SILA and request code access
- Jason to reconnect with Blobfuse/Blockfuse and AC Store teams; review latest materials
- Jai to connect Jason with Lukasz for Bifrost direct-path details
- Jason to verify MAI constraints and current cluster status and report back
- Plan to establish a regular check-in cadence after initial findings
...
- [x] Evaluate OpenAI cache code for viability: legality/access, architecture, quality, performance, scalability to ~100k nodes, and fit with AKS/Spark; compare with Blobfuse/Blockfuse, AC Store/C-Store, and Alexio/DAX @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Confirm OpenAI cache IP/usage rights and request repo/code access with Pete and SILA legal @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Send OpenAI IP note/agreement details to Jason @Jai Menon 🔼 ✅ 2025-10-26
- [x] Send MAI 10-page frustrations document (from Ong) to Jason @Jai Menon 🔼 ✅ 2025-10-26
- [x] Send Apollo document to Jason @Jai Menon 🔼 ✅ 2025-10-26
- [x] Connect Jason with Lukasz to discuss Bifrost direct read path and related implementation details @Jai Menon 🔼 ✅ 2025-10-26
- [x] Review latest materials and status with Blobfuse/Blockfuse (incl. performance numbers), AC Store/C-Store (Krishnan), and Nagendra’s document @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Verify MAI constraints and status (multi-region pooling need, GPU storage cluster readiness, training timelines) and report back @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Discuss performance snapshot outcome with Ong on Friday; consider follow-up conversations with Wamshi and Manish @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Set up a regular cadence of 1-1 check-ins with Jai @Jason Vallery 🔽 ✅ 2025-10-26

## Follow-ups

- [x] Review Jason’s initial cache evaluation findings and choose a direction (OpenAI vs Blobfuse/Blockfuse vs AC Store/C-Store vs Alexio/DAX) @Jai Menon 🔼 ✅ 2025-10-26
- [x] Introduce Jason to Qi Ke and Yumin for AKS/compute alignment if deeper engagement is needed @Jai Menon 🔽 ✅ 2025-10-26
- [x] Schedule a readout/discussion once initial evaluation and MAI requirement confirmations are complete @Jason Vallery 🔽 ✅ 2025-10-26

## Risks

- Unclear IP/license terms and access to OpenAI cache code could delay evaluation
- Achieving required scale (100k nodes; 400k/40k GPUs) may exceed current solutions’ capabilities
- Potential multi-region pooling requirement increases complexity and latency risk
- Fragmentation risk if separate caches are pursued for training vs inference
- Integration complexity with AKS/Spark and existing frameworks
- Cross-org dependencies (Brendan’s org, Qi Ke, MAI) and scheduling (e.g., Wamshi) could slow progress

## Open questions

- Do Microsoft’s IP rights definitively allow access and production use of OpenAI’s cache code across Microsoft services?
- Is OpenAI’s cache unified for training and inference (KB cache), or are there separate cache implementations?
- Can a single, framework-agnostic, pluggable cache meet both training and inference needs at MAI scale?
- Will MAI require multi-region cache pooling over WAN, and what are the latency/consistency implications?
- What is the current maturity and performance of Blobfuse/Blockfuse, AC Store/C-Store, and Alexio/DAX at target scales?
- What is the exact scope and timeline for Bifrost (including the direct read path), and how does it interplay with the cache?
- What is the status of MAI’s first GPU storage clusters and any early results with Blob?
- Should MAI leverage OpenAI’s training framework, and what does that imply for cache integration and productization?
