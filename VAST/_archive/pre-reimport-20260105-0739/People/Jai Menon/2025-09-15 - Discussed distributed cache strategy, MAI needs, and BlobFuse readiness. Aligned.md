...
privacy: "internal"
ai_extracted: true
transcript_path: "00 Inbox/Transcripts/20250915 1531 Teams Meeting (Parallels) Transcription.txt"
tags: [meeting, "1-1"]
---
...
Discussed distributed cache strategy, MAI needs, and BlobFuse readiness. Aligned on evaluating options (build, open source, internal like Manifold/OpenAI TensorCache) and framing scenarios (fan-out writes, fan-out reads, fan-in reads, KV cache). Jai will create a shared Teams room and pursue Manifold briefings; Jason will test BlobFuse PP, compare with alternatives, and craft an MAI-oriented narrative with benchmarks. Offsite confirmed for Oct 15 in SVC; aim to include inferencing/DPU sessions.

## Key facts learned

- Team offsite set for 2025-10-15 in Silicon Valley; day and a half format with light schedule and social/dinner.
- MAI currently writes checkpoints to local NVMe and async-copies to Blob; they favor simplicity.
- BlobFuse PP focus is fan-out writes; limited read caching; recent 100-node test with deployment guide pending.
- Inferencing team showed interest in BlobFuse but has not received bits; two-month delay noted.
- Options: build/fork (BlobFuse/AC Store model), open source (Alluxio, JuiceFS), or internal (Manifold/Singularity, OpenAI TensorCache).
- Manifold claims consistent hashing (no central metadata store); questions remain on scale-in/out rebalancing.
- OpenAI TensorCache reportedly unstable/fast-changing; Whipple presented performance vs TensorCache favoring Manifold.
- Concerns: Alluxio IP/China optics and Java components; BlobFuse perf with Go/FUSE vs potential C++/kernel client.
- Long-term: potential hardware offload/kernel-mode driver for performance; higher risk and longer timeline.

## Outcomes

- Agreed to compare solutions against clear scenarios: fan-out writes, fan-out reads, fan-in reads, and KV cache.
- Plan to gather and centralize collateral (docs, slides, comparisons) in a shared Teams space.
- Intent to produce a concise recommendation after benchmarking BlobFuse PP and reviewing Manifold/TensorCache.
...
- [x] Create a Teams room and add all current distributed caching docs/links; share access with stakeholders (Jason, Jean, Jagan, etc.). @Jai Menon ⏫ ✅ 2025-10-26
- [x] Coordinate with Vipul to schedule a Manifold (Singularity) team briefing and Q&A session. @Jai Menon ⏫ ✅ 2025-10-26
- [x] Share Manifold slides/docs and the Manifold vs TensorCache comparison link in the Teams room. @Jai Menon 🔼 ✅ 2025-10-26
- [x] Obtain BlobFuse PP bits and deployment guide from Vishnu; stand up tests (incl. fan-out writes) and collect perf at target scale. @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Evaluate OpenAI TensorCache and compare with Manifold and BlobFuse on functionality, performance, and stability. @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Restructure the evaluation doc by scenarios (fan-out writes, fan-out reads, fan-in reads, KV cache) and map candidate solutions to each. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Prepare an MAI-focused proposal showing how BlobFuse compares to async copy on performance, complexity, risks, and scale; include benchmarks. @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Set up a requirements call with MAI (with Pete/partners) to validate needs and adoption criteria. @Jason Vallery ⏫ ✅ 2025-10-27
- [x] Analyze metadata strategy trade-offs (consistent hashing vs high-TPS metadata store) and implications for scale-in/out. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Assess BlobFuse implementation choices (Go/FUSE) vs potential C++/kernel client for performance and TTM trade-offs. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Reach out to Rajat Monga to invite an inferencing/KV cache session at the offsite. @Jai Menon 🔼 ✅ 2025-10-26
- [x] Book travel for the Silicon Valley offsite on 2025-10-15. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Share BlobFuse PP bits and guide with the inferencing team once available and collect feedback. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Draft offsite agenda (sessions with DPU/FunOS and inferencing) and circulate. @Jai Menon 🔼 ✅ 2025-10-26

## Follow-ups

- [x] Confirm BlobFuse PP availability and 100-node test results with Vishnu; capture metrics and any bottlenecks. @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Verify Manifold’s approach to scale-in/out with consistent hashing and any rebalancing strategy. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Determine stability and roadmap status of OpenAI TensorCache relevant to near-term evaluations. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Clarify Alluxio IP/China concerns and runtime stack implications for enterprise adoption. @Jason Vallery 🔽 ✅ 2025-10-26
- [x] Confirm attendance/logistics for the Girish storage discussion and adjust travel if needed. @Jai Menon 🔽 ✅ 2025-10-26
- [x] Align on long-term hardware offload/kernel-mode driver vision vs interim user-space path. @Jai Menon 🔼 ✅ 2025-10-26

## Risks

- BlobFuse delivery delays and missed handoffs reduce stakeholder confidence.
- MAI may not adopt BlobFuse without proven perf, scale, and simplicity advantages over async copy.
- Fragmentation: multiple caches for different scenarios increases complexity.
- OpenAI TensorCache instability may block near-term adoption or comparisons.
- Alluxio IP/China optics and Java stack may be unacceptable for some customers.
- Go/FUSE may not meet perf goals, implying costly rework to C++/kernel client.
- Consistent hashing approaches may struggle with dynamic scaling without robust rebalancing.
- Hardware offload/kernel driver path is high risk and long time-to-market.

## Open questions

- Precisely why can’t MAI use BlobFuse today, and what would change that?
- Will BlobFuse meet target performance/scale for MAI vs async copy?
- Is Go/FUSE sufficient long term, or do we need C++/kernel client?
- How do Manifold/OpenAI handle dynamic scaling and data rebalancing without a central metadata store?
- Do we build/own a high-TPS metadata store, or leverage existing OSS (e.g., FoundationDB)?
- Are Alluxio’s IP/China optics and Java components acceptable for our customers?
- Will the inferencing team adopt BlobFuse once PP bits and guide are available?
- If MAI and OpenAI won’t adopt, who is the primary target customer for this cache?
