...
Jai outlined a high-priority evaluation for an AI caching strategy to support MAI scale. Options include OpenAI’s cache IP, Alluxio/DAX, proposals around C-Store, and Blockfuse/BlobFuse approaches. Primary ask: Jason to validate feasibility of using OpenAI’s cache (IP access, code quality, scalability to ~100k nodes, and fit with AKS/Spark). Training is first priority, with a preference for a single cache spanning training and inference. Bifrost is the current Blob direction with a direct read path; DeltaZero is the potential follow-on. Jason will also reconnect with MAI (Ong) on requirements and status, and with Lukasz on Bifrost. Jai will send relevant docs (OpenAI IP note, MAI pain points, Apollo).

## Key facts learned

- Performance snapshot outcome appears to be Meets Expectations; Jason disappointed and will discuss with Ong and potentially Manish.
- MAI scale targets: ~400k GPUs for training and ~40k GPUs for inference in ~2 years.
- Target data-plane scale: ~100k nodes; environment: AKS + Spark.
- Potential multi-region, cross-WAN cache pooling may be required (to confirm with MAI).
- Caching options: OpenAI cache IP; Alluxio/DAX (now supports inference/KB caching); proposals around C-Store; Blockfuse/BlobFuse work in progress.
- Strategic preference: one caching solution usable for both training and inference; pluggable, not tightly coupled to a single framework.
- Bifrost direction includes a direct read path bypassing FE/table for reads; Lukasz is building parts of this.
- DeltaZero positioned as a follow-on to Bifrost (work in progress).
- Compute for MAI moved to Brendan’s AKS org; a CVP (Qi/“Kiki” Ke) is leading compute side; Yumin interfacing from storage side.

## Outcomes

- Agreed that Jason will lead an initial evaluation of OpenAI’s cache IP for feasibility at MAI scale and fit with AKS/Spark.
- Jai will provide documents: OpenAI IP/usage note, MAI pain points (10 pages), and Apollo doc.
- Jason will reconnect with Ong (Friday) to clarify MAI requirements and raise snapshot concerns.
- Jason will sync with Lukasz to understand Bifrost direct-read path.
- Plan to establish a regular 1:1 rhythm between Jai and Jason.
...
- [x] Evaluate OpenAI cache feasibility (confirm IP access, obtain code, assess architecture, performance, and operational fit for ~100k nodes and AKS/Spark; training first, inference later). @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Confirm legal/IP rights with Pete and SILA and arrange access to OpenAI cache code. @Jason Vallery ⏫ ✅ 2025-10-26
- [x] Review latest Blockfuse/BlobFuse progress materials (e.g., Nagendra’s document) and status. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Meet Ong to clarify MAI requirements (scale, regions, timelines) and discuss performance snapshot. @Jason Vallery ⏫ 📅 2025-09-05 ✅ 2025-10-26
- [x] Sync with Lukasz on Bifrost direct-read path design and status. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] Draft an initial recommendation comparing OpenAI cache vs Alluxio/DAX, C-Store proposals, and Blockfuse/BlobFuse for MAI needs. @Jason Vallery ⏫ ✅ 2025-10-26

## Follow-ups

- [x] Send OpenAI IP/usage note and contact details; share MAI pain-points doc (10 pages); share Apollo doc. @Jai Menon ⏫ ✅ 2025-10-26
- [x] Coordinate with Wamshi and SILA as needed after initial IP check. @Jason Vallery 🔼 ✅ 2025-10-26
- [x] After meeting Ong, decide whether to escalate snapshot discussion to Manish. @Jason Vallery 🔼 📅 2025-09-05 ✅ 2025-10-26
- [x] Propose a regular 1:1 cadence with Jai. @Jason Vallery 🔽 ✅ 2025-10-26

## Risks

- IP/access uncertainty for OpenAI cache code may delay evaluation.
- Scalability to ~100k nodes and multi-region operation may be challenging.
- Integration complexity with AKS/Spark and varied training/inference frameworks.
- Blob latency/throughput may remain a bottleneck without timely Bifrost/DeltaZero improvements.
- Org/ownership shifts across teams could slow coordination.
- Morale/engagement risk due to performance snapshot outcome.

## Open questions

- Do Microsoft’s IP agreements fully permit use of OpenAI’s cache code across services, and can we get timely access?
- Can the OpenAI cache scale to ~100k nodes and meet MAI’s performance/SLA needs?
- How well does the OpenAI cache integrate with AKS and Spark operationally?
- Is OpenAI’s caching unified for training and inference, or separate implementations?
- Does MAI require multi-region, cross-WAN cache pooling for initial clusters?
- What is the current maturity and performance of Blockfuse/BlobFuse and C-Store proposals?
- What is the status/timeline of MAI’s first GPU storage clusters and training start?
- How will Bifrost and DeltaZero sequencing impact near-term latency/throughput goals?
